...
public function salvar($p)
{
  ...
  try {
    ...
    $time = strtotime(str_replace('/', '-', $p['dt_cadastro']));

    
    $date = date('d-m-Y',$time); 
    $dataAtual =  strtotime(date("d-m-Y H:i:s"));

    $u =  Usuario::findOne(Yii::$app->user->identity->idusuario);
    ...
    ->insert('acompanhamento', [
      ...
      'trecho_idtrecho' => $u->grupoIdgrupos->trecho_idtrecho,
      'dt_cadastro' => $date,
...
function buscaResultados($model)
{

  $inicio = strtotime(str_replace('/', '-', $model->inicio));
  $fim = strtotime(str_replace('/', '-', $model->final));


  $a = Acompanhamento::find()
  ->joinWith('trechoIdTrecho')
  ->joinWith('acompanhamentoPerguntaRespostas')
  ->where(['trecho_idtrecho' => $model->trecho])
  ->andWhere(['between', 'dt_cadastro', $inicio, $fim])
  ->orderBy('dt_cadastro')
  -> all();
